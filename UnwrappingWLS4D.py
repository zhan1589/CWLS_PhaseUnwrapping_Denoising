...
class PhaseUnwrappingWeightedLeastSquares4D():
  ...
  def __init__(self,Xn,Yn,Zn,Un,Vn,Wn,fluid_maskn,unwrapping_maskn):
    """
  ...
  def phase_diff_error_divergence(self):
    ...
    # Evaluate the divergence field from the filtered phase differences with SOC-type scheme.
    div_field = np.full(self.fluid_maskn.shape, np.nan)
    div_field[1:-1,1:-1,1:-1] = (phase_u_diff_x[1:-1,1:,1:-1] + phase_u_diff_x[1:-1,:-1,1:-1])/(2.0*self.dx)
    div_field[1:-1,1:-1,1:-1] += (phase_v_diff_y[1:,1:-1,1:-1] + phase_v_diff_y[:-1,1:-1,1:-1])/(2.0*self.dy)
    div_field[1:-1,1:-1,1:-1] += (phase_w_diff_z[1:-1,1:-1,1:] + phase_w_diff_z[1:-1,1:-1,:-1])/(2.0*self.dz)
    ...
    phase_error_vect,istop,_,_,_,_,_,_,_,_ = splinalg.lsqr(Opt_div_aug, div_field_vect)
    ...
    self.Phase_error_div['U'][ju,iu,ku] = phase_error_vect[:Npts_u]
    self.Phase_error_div['V'][jv,iv,kv] = phase_error_vect[Npts_u:Npts_u+Npts_v]
    self.Phase_error_div['W'][jw,iw,kw] = phase_error_vect[Npts_u+Npts_v:]
  ...
  def construct_linear_system_WLS(self):
    ...
    weights_vector_div = np.ones(Npts)*1e6*self.mean_weights_grad
  ...
  def solve_linear_system_WLS_lsqr(self, Phase_u_0=None, Phase_v_0=None, Phase_w_0=None):
    ...
    self.phase_vect,istop,_,_,_,_,_,_,_,_ = splinalg.lsqr(self.Operator_augmented,self.rhs_augmented,x0=x0)
